<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Glasaj.aspx.cs" Inherits="lab1v.Glasaj" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
</head>
<body>
    <form id="form1" runat="server">
        <div>
            <img src="data:image/png;base64," alt="Медиа | ФИНКИ"/>
            <asp:Label ID="ime" runat="server" Text=""></asp:Label>
            <br />
            <asp:ListBox ID="predmetils" runat="server" OnSelectedIndexChanged="ListBox1_SelectedIndexChanged" AutoPostBack="True"></asp:ListBox>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" ControlToValidate="predmetils" ErrorMessage="Мора да имаш селектирано предмет"></asp:RequiredFieldValidator>
            <asp:ListBox ID="kreditils" runat="server"></asp:ListBox>
            <br />
            <asp:Button ID="glas" runat="server" Text="Гласајте" OnClick="glas_Click" />
            <hr/>
            <br />
            Predmet:<asp:TextBox ID="p" runat="server"></asp:TextBox>
            <br />
            Krediti:<asp:TextBox ID="k" runat="server"></asp:TextBox>
            <br />
            <asp:Button ID="dodadi" runat="server" Text="Додади" OnClick="dodadi_Click" />
            <asp:Button ID="izbrisi" runat="server" Text="Избриши" OnClick="izbrisi_Click" />
        </div>
    </form>
</body>
</html>
